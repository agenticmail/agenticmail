...
- **🏠 Local host voice bridge.** `agenticmail-voice-host-bridge` runs the OpenAI-Realtime-compatible provider connection in the host process, so AgenticMail can use `AGENTICMAIL_VOICE_RUNTIME=host_bridge` without storing the live model-provider key. OpenClaw, Codex, and Claude Code packages expose matching wrapper bins.
...
| `agenticmail-voice-host-bridge` | **Run the localhost host-owned realtime bridge.** Use with `AGENTICMAIL_VOICE_RUNTIME=host_bridge` and `AGENTICMAIL_VOICE_HOST_BRIDGE_URL=ws://127.0.0.1:3999/realtime` so OpenClaw/Codex/Claude Code can own the live provider key. |
...
After it finishes, you restart Claude Code and you've got 100 `mcp__agenticmail__*` tools plus one Claude Code subagent per AgenticMail agent.
