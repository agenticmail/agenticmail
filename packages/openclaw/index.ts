...
function activate(api: any): void {
  ...
  api.on('before_agent_start', async (_event: any, context: any) => {
    ...
    prependLines.push(
      ...
      '- agenticmail_assign_task: Assign work to another agent (they get notified instantly via push)',
      '- agenticmail_check_tasks: Check task status — pending, claimed, or completed with results',
      '- agenticmail_claim_task: Claim a pending task to start working on it',
      '- agenticmail_submit_result: Submit structured JSON results when done',
      '',
      'SYNCHRONOUS RPC (when you need a result back immediately):',
      '- agenticmail_call_agent: Call another agent, wait for their structured response (up to 5 min)',
      '',
      ...
      '- agenticmail_wait_for_email: Wait for a reply efficiently (push-based SSE, not polling)',
      '',
      'WHY THIS IS BETTER THAN sessions_spawn/sessions_send:',
      '- Persistent: Tasks survive agent crashes and restarts (stored in DB, not memory)',
      '- Structured: JSON results with status lifecycle (pending → claimed → completed)',
      '- Push-based: No wasted polling — get notified instantly when work completes',
      '- Discoverable: Agents find each other by name and role (no hardcoded session keys)',
      '- Auditable: Every coordination action is an email — natural audit trail',
      '- ~60% fewer tokens on multi-agent tasks (eliminates redundant polling cycles)',
